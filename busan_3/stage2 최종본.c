...
int temp_villain, villain, villain_aggro; //빌런 관련 함수
// 인트로 함수
void print_intro() {
    printf("********************\n");
    printf("**  부산행 게임   **\n");
...
void zombie_result() {
    //좀비가 아무도 공격안했을때
    if (zombie != citizen + 1 && zombie != villain + 1) {
        printf("zombie attacked nobody.\n");
    }
    else if (zombie == villain + 1) {
        printf("zombie attacked villain.\n");
    }
}
//마동석의 스테미나 0
...
int main() {
    ...
    while (1) {
        ...
        // 시민과 좀비의 움직임 계산
        move_citizen_villain(); // 시민의 움직임 계산
